    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Compre e ganhe dinheiro de volta
        </p>
      </div>
      ...
      <Card className="p-6 bg-gradient-primary text-white">
        <div className="space-y-4">
          ...
          <Button
            ...
            onClick={async () => {
              try {
                await cashbackService.redeemCashback(totalCashback);
                setTotalCashback(0);
                // Recarregar os dados
                // const cashbackData = await cashbackService.getCashbackData();
                // setTotalCashback(cashbackData.balance);
              } catch (error) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Faltam R$ {(nextReward - totalCashback).toFixed(2)} para seu
                próximo bônus
              </p>
            </div>
      ...
      <div>
        ...
        <div className="grid gap-4">
          {offers.map((offer, index) => (
            <Card
              key={index}
              className="p-6 hover:border-primary transition-all cursor-pointer hover:scale-[1.02]"
            >
              <div className="flex items-center justify-between">
